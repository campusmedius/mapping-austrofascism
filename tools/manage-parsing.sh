...
# ARG_TYPE_GROUP_SET([command],[COMMAND],[command],[env, init, run, deploy, ssh, upgrade, build, backup])
...
command()
{
	local _allowed=("env" "init" "run" "deploy" "ssh" "upgrade" "build" "backup") _seeking="$1"
	for element in "${_allowed[@]}"
	do
		test "$element" = "$_seeking" && echo "$element" && return 0
	done
	die "Value '$_seeking' (of argument '$2') doesn't match the list of allowed values: 'env', 'init', 'run', 'deploy', 'ssh', 'upgrade', 'build' and 'backup'" 4
}
...
print_help()
{
	...
	printf '\t%s\n' "<command>: .. Can be one of: 'env', 'init', 'run', 'deploy', 'ssh', 'upgrade', 'build' and 'backup'"
